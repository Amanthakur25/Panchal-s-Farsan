...
const SnacksPage = () => {
  ...
  const snacksProducts = products.filter(product => 
    ...
    product.category.toLowerCase().includes('savory')
  );
  ...
  const subCategories = ['all', ...new Set(snacksProducts.map(p => p.subCategory).filter(Boolean) as string[])];

  // Apply filters
  let filteredProducts = [...snacksProducts];
