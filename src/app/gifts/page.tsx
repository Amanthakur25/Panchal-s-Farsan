...
const GiftsPage = () => {
  ...
  const giftsProducts = products.filter(product => 
    product.category.toLowerCase() === 'gifts' || 
    product.subCategory?.toLowerCase().includes('gift') ||
  ...
  const subCategories = ['all', ...new Set(giftsProducts.map(p => p.subCategory).filter(Boolean) as string[])];

  // Apply filters
  let filteredProducts = [...giftsProducts];
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6 text-sm text-gray-600">
        ...
        <span className="text-gray-900 font-medium">Gifts</span>
      </div>
